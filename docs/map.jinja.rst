...
We will see a quick setup to configure the ``tool_hammerspoon`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_hammerspoon`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_hammerspoon`` formulas under ``/srv/salt/tool_hammerspoon/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_hammerspoon/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_hammerspoon/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_hammerspoon/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_hammerspoon/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_hammerspoon`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_hammerspoon`` formulas under ``/srv/salt/tool_hammerspoon/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_hammerspoon/parameters/roles

We will define 2 roles:

- ``tool_hammerspoon/server``
- ``tool_hammerspoon/client``

We create a configuration for the role ``tool_hammerspoon/server`` in ``/srv/salt/tool_hammerspoon/parameters/roles/tool_hammerspoon/server.yaml``:
...
We create another configuration for the role ``tool_hammerspoon/client`` in ``/srv/salt/tool_hammerspoon/parameters/roles/tool_hammerspoon/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_hammerspoon/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_hammerspoon/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_hammerspoon/parameters/dns:domain/  '/srv/salt/tool_hammerspoon/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-hammerspoon-as-json:
      test.nop:
        - name: {{ hammerspoon | json }}
...
.. code-block:: sls
    ...
    hammerspoon-config-file-file-managed:
      file.managed:
        - name: {{ hammerspoon.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='hammerspoon-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ hammerspoon.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            hammerspoon: {{ hammerspoon | json }}

This ``sls`` file expose a ``hammerspoon`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ hammerspoon['winner'] }}
